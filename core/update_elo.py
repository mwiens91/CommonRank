...
def update_elo(winner, loser, elo_sensitivity):
    ...
    if(winner_games<GAMES_IN_PROBATION and winner_games>=GAMES_IN_PROBATION//2):
        winner_k_factor = winner_k_factor*2
    elif(winner_games<GAMES_IN_PROBATION//2 and winner_games>=0):
        winner_k_factor = winner_k_factor*4
    if(loser_games<GAMES_IN_PROBATION and loser_games>=GAMES_IN_PROBATION//2):
        loser_k_factor = loser_k_factor*2
    elif(loser_games<GAMES_IN_PROBATION//2 and loser_games>=0):
        loser_k_factor = loser_k_factor*4

    loser.elo += loser_k_factor*(0-expected_loser)
    winner.elo += winner_k_factor*(1-expected_winner)
